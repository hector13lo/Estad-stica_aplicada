...
for (df in dfs) {
  ...
  medias_muestrales <- numeric(length(sample_sizes))
  varianzas_muestrales <- numeric(length(sample_sizes))
  ...
  df_media <- data.frame(Tamaño = sample_sizes, Media = medias_muestrales)
  df_varianza <- data.frame(Tamaño = sample_sizes, Varianza = varianzas_muestrales)
  

  p1 <- ggplot(df_media, aes(x = Tamaño, y = Media)) +
    geom_line(color = "lightgreen") +
    geom_point(color = "green") +
    geom_hline(yintercept = mean_theoretical, linetype = "dashed", color = "orange") +
    scale_x_log10() +
    labs(title = paste("Convergencia de la Media (χ², df =", df, ")"),
         x = "Tamaño de muestra (escala log)",
         y = "Media muestral") +
    theme_minimal()
 
  p2 <- ggplot(df_varianza, aes(x = Tamaño, y = Varianza)) +
    geom_line(color = "grey") +
    geom_point(color = "black") +
    geom_hline(yintercept = var_theoretical, linetype = "dashed", color = "red") +
    scale_x_log10() +
    labs(title = paste("Convergencia de la Varianza (χ², df =", df, ")"),
         x = "Tamaño de muestra (escala log)",
         y = "Varianza muestral") +
    theme_minimal()
  ...
  muestra_grande <- rchisq(n_max, df = df)
  df_hist <- data.frame(x = muestra_grande)
  
  p3 <- ggplot(df_hist, aes(x = x)) +
    geom_histogram(aes(y = ..density..), bins = 50, fill = "lightblue", alpha = 0.7, color = "grey") +
    stat_function(fun = dchisq, args = list(df = df), color = "orange", size = 1) +
    labs(title = paste("Histograma vs Densidad Teórica (χ², df =", df, ", n =", n_max, ")"),
         x = "Valores",
         y = "Densidad") +
    theme_minimal()
...
df <- 15                    
num_sim <- 500            
sample_sizes <- c(2, 5, 25, 50, 150, 250) 
...
for (n in sample_sizes) {
  ...
  z <- (medias - media_teorica) / sqrt(var_teorica / n)
...
df_tcl <- do.call(rbind, resultados)


ggplot(df_tcl, aes(x = Z, fill = Tamaño)) +
  geom_histogram(aes(y = ..density..), bins = 50, alpha = 0.6, color = "grey") +
  stat_function(fun = dnorm, args = list(mean = 0, sd = 1),
                color = "lightgreen", size = 1.2) +
  facet_wrap(~Tamaño, scales = "free") +
  labs(
    title = "Verificación del Teorema Central del Límite (TCL)",
    subtitle = paste("Distribución Ji-Cuadrada con df =", df),
    x = "Valor estandarizado Z",
    y = "Densidad"
  ) +
  theme_minimal()
...
x_vals <- seq(0, max(X_sim), length.out = 500)
dens_df <- data.frame(x = x_vals, dens = dchisq(x_vals, df = k))

# Gráfico comparativo
ggplot(data.frame(X_sim), aes(x = X_sim)) +
  geom_histogram(aes(y = ..density..), bins = 50, fill = "lightblue", color = "black", alpha = 0.7) +
  geom_line(data = dens_df, aes(x = x, y = dens), color = "yellow", size = 1.2) +
  labs(title = paste("Propiedad de Aditividad de χ² (df =", k, ")"),
       subtitle = "Simulación de suma de cuadrados de normales estándar",
       x = "Valores simulados",
       y = "Densidad") +
  theme_minimal()
...
cat("Media teórica:", k, "\n")
cat("Media simulada:", round(mean(X_sim),3), "\n")
cat("Varianza teórica:", 2*k, "\n")
cat("Varianza simulada:", round(var(X_sim),3), "\n")
...
for (df in dfs) {
  ...
  df_hist <- data.frame(x = muestra)
  p_hist <- ggplot(df_hist, aes(x = x)) +
    geom_histogram(aes(y = ..density..), bins = 50, fill = "lightgreen", alpha = 0.6, color = "yellow") +
    stat_function(fun = dnorm, args = list(mean = mu, sd = sigma),
                  color = "grey", size = 1.2) +
    labs(title = paste("Histograma χ² df =", df, "vs Normal Teórica"),
         x = "Valores", y = "Densidad") +
    theme_minimal()
  
  plots_hist[[as.character(df)]] <- p_hist
  
 
  z_samp <- (muestra - mu)/sigma
  df_qq <- data.frame(z = z_samp)
  p_qq <- ggplot(df_qq, aes(sample = z)) +
    stat_qq() +
    stat_qq_line(color = "darkblue") +
    labs(title = paste("QQ-plot estandarizado χ² df =", df),
         x = "Teórica N(0,1)", y = "Simulada") +
    theme_minimal()
  
  plots_qq[[as.character(df)]] <- p_qq
  

  cat("df =", df, ": Media simulada =", round(mean(muestra),3),
      " Varianza simulada =", round(var(muestra),3),
      " Media teórica =", mu,
      " Varianza teórica =", sigma^2, "\n")
}

do.call(grid.arrange, c(plots_hist, ncol = 2))
do.call(grid.arrange, c(plots_qq, ncol = 2))
...
X1 <- rchisq(n_sim, df = df1)
X2 <- rchisq(n_sim, df = df2)
...
x_vals <- seq(0, max(S), length.out = 200)
dens_teorica <- dchisq(x_vals, df = df_sum)

dens_df <- data.frame(x = x_vals, dens = dens_teorica)

ggplot(data.frame(S), aes(x = S)) +
  geom_histogram(aes(y = ..density..), bins = 50,
                 fill = "skyblue", color = "grey", alpha = 0.7) +
  geom_line(data = dens_df, aes(x = x, y = dens), color = "orange", size = 1.2) +
  labs(title = paste("Suma de χ² independientes: df1 =", df1, "df2 =", df2),
       subtitle = paste("Comparación con χ² teórica df =", df_sum),
       x = "Valores simulados",
       y = "Densidad") +
  theme_minimal()
...
cat("Suma teórica: df =", df_sum, "\n")
cat("Media teórica:", df_sum, " | Media simulada:", round(media_sim,3), "\n")
cat("Varianza teórica:", 2*df_sum, " | Varianza simulada:", round(var_sim,3), "\n")
