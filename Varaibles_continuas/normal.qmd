...
ggplot(dist_altura)+
  geom_line(aes(estaturas, pdf_altura), color = "orange", linewidth=1.3)+
  geom_area(data = dist_altura |> filter(estaturas >=180), aes(estaturas, pdf_altura), fill= "blue", alpha=0.5)+
  geom_vline(xintercept = 180, linetype ="dashed")+
  annotate("label", x=190, y=0.01, 
           label = paste("1-F(180)= ", round(pnorm(180, mu_altura, sigma_altura, lower.tail = FALSE), 4) ), color = "grey", size=3, fill = "lightgreen")+
  theme_bw()
...
```{r}
ggplot()+
  geom_histogram(aes(muestra_altura, y=after_stat(density)), fill = "lightblue", color= "blue", center = 170)+
  geom_line(data = dist_altura, aes(estaturas, pdf_altura), color = "yellow", linewidth=1.3)+
  theme_bw()
...
cat("Porcentaje fuera de especificación:", fuera_especificacion, "%\n")


mu_descentrado <- 355
temperaturas_descentradas <- rnorm(n, mean = mu_descentrado, sd = sigma)
fuera_especificacion_descentrado <- mean(temperaturas_descentradas < lim_inf | temperaturas_descentradas > lim_sup) * 100
cat("Porcentaje fuera de especificación con proceso descentrado:", fuera_especificacion_descentrado, "%\n")



z <- qnorm(0.9995)  # percentil 99.95% de la normal
sigma_requerido <- 10 / z
cat("Desviación estándar requerida para 99.9% dentro de especificación:", round(sigma_requerido, 3), "\n")


cat("\nResumen:\n")
cat("Sigma actual:", sigma, "\n")
cat("Sigma requerido (para 99.9% dentro):", round(sigma_requerido, 3), "\n")
cat("Porcentaje fuera (mu=350):", round(fuera_especificacion, 2), "%\n")
cat("Porcentaje fuera (mu=355):", round(fuera_especificacion_descentrado, 2), "%\n")
...
ventas <- rnorm(n_meses, mean = mu_ventas, sd = sigma_ventas)
summary(ventas)


p_mayor_60000 <- 1 - pnorm(60000, mean = mu_ventas, sd = sigma_ventas)
cat("Probabilidad de ventas > 60000:", round(p_mayor_60000, 4), "\n")


p_menor_30000 <- pnorm(30000, mean = mu_ventas, sd = sigma_ventas)
meses_menor_30000 <- n_meses * p_menor_30000
cat("Meses esperados con ventas < 30000:", round(meses_menor_30000, 2), "\n")


IQR_ventas <- IQR(ventas)
cat("Rango intercuartílico de las ventas:", round(IQR_ventas, 2), "\n")


porcentaje_meta <- mean(ventas > 45000) * 100
cat("Porcentaje de meses con ventas > 45000:", round(porcentaje_meta, 2), "%\n")
```
...
vida_bat <- rnorm(n_bat, mean = mu_bat, sd = sigma_bat)


garantia_95 <- qnorm(0.05, mean = mu_bat, sd = sigma_bat, lower.tail = FALSE)
cat("Garantía (95% de las baterías cubiertas):", round(garantia_95, 2), "horas\n")


p_falla_900 <- pnorm(900, mean = mu_bat, sd = sigma_bat)
fallas_900 <- 10000 * p_falla_900
cat("Baterías que fallan con garantía de 900h:", round(fallas_900, 0), "\n")


cat("Media teórica:", mu_bat, " | Media simulada:", round(mean(vida_bat), 2), "\n")
cat("Mediana teórica:", mu_bat, " | Mediana simulada:", round(median(vida_bat), 2), "\n")


percentil_10 <- qnorm(0.10, mean = mu_bat, sd = sigma_bat)
cat("Percentil 10 teórico:", round(percentil_10, 2), "\n")
```
...
errores <- rnorm(n_med, mean = mu_error, sd = sigma_error)


p_entre_5 <- mean(errores >= -5 & errores <= 5) * 100
cat("Porcentaje de errores entre -5 y +5:", round(p_entre_5, 2), "%\n")


p_mayor_6 <- (1 - pnorm(6, mean = mu_error, sd = sigma_error)) * 2
cat("Probabilidad de error > 6 unidades (|error|>6):", round(p_mayor_6, 4), "\n")


p_preciso <- mean(abs(errores) <= 3) * 100
cat("Porcentaje de mediciones precisas (|error| ≤ 3):", round(p_preciso, 2), "%\n")


cat("Media de errores:", round(mean(errores), 3), "\n")
cat("Mediana de errores:", round(median(errores), 3), "\n")
```
...
grupo_A <- rnorm(n_A, mean = mu_A, sd = sigma_A)
grupo_B <- rnorm(n_B, mean = mu_B, sd = sigma_B)


porc_A_supera_B <- mean(grupo_A > mean(grupo_B)) * 100
cat("Porcentaje Grupo A > media Grupo B:", round(porc_A_supera_B, 2), "%\n")


p_B_menor_70 <- pnorm(70, mean = mu_B, sd = sigma_B)
cat("Probabilidad que Grupo B tenga puntuación < 70:", round(p_B_menor_70, 4), "\n")


hist(grupo_A, breaks = 20, col = rgb(1, 0, 0, 0.4), xlim = c(40, 110),
     main = "Distribución de Puntuaciones", xlab = "Puntaje")
hist(grupo_B, breaks = 20, col = rgb(0, 0, 1, 0.4), add = TRUE)
...
cat("10% superior Grupo A a partir de:", round(p90_A, 2), "\n")
cat("10% superior Grupo B a partir de:", round(p90_B, 2), "\n")
```
