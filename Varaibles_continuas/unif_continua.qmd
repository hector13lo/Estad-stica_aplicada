...
hist(tiempos, 
     probability = TRUE, 
     main = "Distribución del Tiempo de Carga (Uniforme 2-8)",
...
# 4. Probabilidad de que cargue en menos de 4 segundos
prob_menor_4 <- punif(4, min = a, max = b)
prob_menor_4


cat("Percentil 75 =", percentil_75, "segundos\n")
cat("Probabilidad de cargar en menos de 4 segundos =", prob_menor_4, "\n")
...
set.seed(123)
calificaciones <- runif(n, min = a, max = b)

# 2. Porcentaje de estudiantes con calificación > 85
porc_mayor_85 <- mean(calificaciones > 85) * 100
porc_mayor_85
...
# Porcentaje en cada categoría (simulado)
porc_simulado <- prop.table(table(categorias)) * 100
porc_simulado

# 4. Porcentajes teóricos (para una Uniforme(70, 100))
# P(70–79) = (80-70)/(100-70)
# P(80–89) = (90-80)/(100-70)
# P(90–100) = (100-90)/(100-70)

porc_teorico <- c(
  Aprobado = (80 - 70) / (100 - 70) * 100,
  Bien = (90 - 80) / (100 - 70) * 100,
  Excelente = (100 - 90) / (100 - 70) * 100
)
porc_teorico
...
hist(calificaciones,
     ...
     col = "lightblue",
     border = "white",
     main = "Distribución de Calificaciones (Uniforme 70–100)",
     xlab = "Calificación")

# Densidad teórica
curve(dunif(x, min = a, max = b),
      from = a, to = b, col = "red", lwd = 2, add = TRUE)


abline(v = c(80, 90), col = "darkgreen", lwd = 2, lty = 2)
text(x = c(75, 85, 95), y = 0.035, 
     labels = c("Aprobado", "Bien", "Excelente"), 
     col = "darkgreen")

# 6. Mostrar resultados
cat("Porcentaje con calificación > 85:", porc_mayor_85, "%\n\n")
cat("Porcentajes simulados por categoría:\n")
print(porc_simulado)
...
# 1. Crear la distribución truncada (sesgada)

valores <- runif(100000, 0, 1)
valores <- valores[valores > 0.3]


hist(valores, probability = TRUE, 
     col = "lightblue", border = "white",
     main = "Distribución Original Truncada (x > 0.3)",
     xlab = "x")

# 2. tamaños de muestra
n_muestras <- c(5, 15, 30, 60, 120)
num_reps <- 800
...
for (n in n_muestras) {
  medias <- replicate(num_reps, mean(sample(valores, n, replace = TRUE)))
  
  # Normalización de las medias (Z = (x - mean) / sd)
  medias_norm <- scale(medias)
  
  # Calcular skewness y kurtosis
  asimetria <- skewness(medias)
  ...
  hist(medias_norm,
       ...
       col = "lightgreen",
       border = "white",
       main = paste("n =", n, "- Distribución de Medias Normalizadas"),
...
# 4. Mostrar resultados de asimetría y curtosis
cat("Resultados de Skewness y Kurtosis:\n\n")
for (n in names(resultados)) {
  cat("n =", n, "\n")
  cat("   Skewness:", round(resultados[[n]]$skewness, 4), "\n")
  cat("   Kurtosis:", round(resultados[[n]]$kurtosis, 4), "\n\n")
}
...
# 5. Mostrar resultado
cat("Estimación de pi usando Monte Carlo =", pi_estimado, "\n")


plot(x, y, col = ifelse(dentro_circulo, "blue", "red"),
     pch = 16, cex = 0.5,
     main = "Estimación de π por Monte Carlo",
     xlab = "x", ylab = "y")
