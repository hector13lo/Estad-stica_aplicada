...
cat("=== Propiedad a) X ~ t(n) => X² ~ F(1,n) ===\n")

n <- 10  # grados de libertad para t
grados_libertad_t <- n

# Generar muestras de distribución t
muestras_t <- rt(n_muestras, df = n)
cuadrados_t <- muestras_t^2

# Crear gráfico para propiedad a)
par(mfrow = c(1, 2))

# Histograma de X²
hist(cuadrados_t, breaks = 50, freq = FALSE, 
     main = expression(paste("X"^2, " donde X ~ t(", n, ")")),
     xlab = expression(X^2), ylab = "Densidad",
     col = "lightblue", xlim = c(0, 15))

# Superponer densidad teórica F(1,n)
x_seq <- seq(0, 15, length.out = 1000)
densidad_F <- df(x_seq, df1 = 1, df2 = n)
lines(x_seq, densidad_F, col = "red", lwd = 2)

legend("topright", legend = c("Simulación", "F(1,n) teórica"),
       col = c("lightblue", "red"), lwd = c(10, 2), bty = "n")

# QQ-plot para comparar distribuciones
qqplot(cuadrados_t, rf(n_muestras, df1 = 1, df2 = n),
       main = "QQ-plot: X² vs F(1,n)",
       xlab = "Cuantiles X²", ylab = "Cuantiles F(1,n)")
abline(0, 1, col = "red", lwd = 2)

# Prueba KS
ks_test_a <- ks.test(cuadrados_t, "pf", df1 = 1, df2 = n)
cat("Prueba KS - p-value:", ks_test_a$p.value, "\n")
cat("¿Se cumple la propiedad?:", ks_test_a$p.value > 0.05, "\n\n")
...
# Generar muestras de F(a,b)
muestras_F <- rf(n_muestras, df1 = a, df2 = b)
inversas_F <- 1/muestras_F

# Crear gráfico para propiedad b)
par(mfrow = c(1, 2))

# Histograma de 1/X
hist(inversas_F, breaks = 50, freq = FALSE, 
     main = expression(paste("1/X donde X ~ F(", a, ",", b, ")")),
     xlab = "1/X", ylab = "Densidad",
     col = "lightgreen", xlim = c(0, 8))

# Superponer densidad teórica F(b,a)
x_seq <- seq(0, 8, length.out = 1000)
densidad_F_inv <- df(x_seq, df1 = b, df2 = a)
lines(x_seq, densidad_F_inv, col = "blue", lwd = 2)

legend("topright", legend = c("Simulación 1/X", "F(b,a) teórica"),
       col = c("lightgreen", "blue"), lwd = c(10, 2), bty = "n")

# QQ-plot
qqplot(inversas_F, rf(n_muestras, df1 = b, df2 = a),
       main = "QQ-plot: 1/X vs F(b,a)",
       xlab = "Cuantiles 1/X", ylab = "Cuantiles F(b,a)")
abline(0, 1, col = "blue", lwd = 2)

# Prueba KS
ks_test_b <- ks.test(inversas_F, "pf", df1 = b, df2 = a)
cat("Prueba KS - p-value:", ks_test_b$p.value, "\n")
cat("¿Se cumple la propiedad?:", ks_test_b$p.value > 0.05, "\n\n")

# ----------------------------
# c) Si X ~ χ²(a) e Y ~ χ²(b) son independientes, entonces (X/a)/(Y/b) ~ F(a,b)
# ----------------------------

cat("=== Propiedad c) (X/a)/(Y/b) ~ F(a,b) donde X~χ²(a), Y~χ²(b) ===\n")

a_chi <- 4
b_chi <- 6

# Generar muestras independientes de chi-cuadrado
X <- rchisq(n_muestras, df = a_chi)
Y <- rchisq(n_muestras, df = b_chi)

# Calcular (X/a)/(Y/b)
F_calculado <- (X/a_chi) / (Y/b_chi)

# Crear gráfico para propiedad c)
par(mfrow = c(1, 2))

# Histograma de F calculado
hist(F_calculado, breaks = 50, freq = FALSE, 
     main = expression(paste("(X/a)/(Y/b) ~ F(", a, ",", b, ")")),
     xlab = "(X/a)/(Y/b)", ylab = "Densidad",
     col = "lightcoral", xlim = c(0, 10))

# Superponer densidad teórica F(a,b)
x_seq <- seq(0, 10, length.out = 1000)
densidad_F_teorica <- df(x_seq, df1 = a_chi, df2 = b_chi)
lines(x_seq, densidad_F_teorica, col = "darkred", lwd = 2)

legend("topright", legend = c("Simulación", paste0("F(", a_chi, ",", b_chi, ") teórica")),
       col = c("lightcoral", "darkred"), lwd = c(10, 2), bty = "n")

# QQ-plot
qqplot(F_calculado, rf(n_muestras, df1 = a_chi, df2 = b_chi),
       main = "QQ-plot: (X/a)/(Y/b) vs F(a,b)",
       xlab = "Cuantiles (X/a)/(Y/b)", ylab = "Cuantiles F(a,b)")
abline(0, 1, col = "darkred", lwd = 2)

# Prueba KS
ks_test_c <- ks.test(F_calculado, "pf", df1 = a_chi, df2 = b_chi)
cat("Prueba KS - p-value:", ks_test_c$p.value, "\n")
cat("¿Se cumple la propiedad?:", ks_test_c$p.value > 0.05, "\n")
...
# Resumen final
cat("\n=== RESUMEN FINAL ===\n")
cat("Propiedad a):", ifelse(ks_test_a$p.value > 0.05, "✓ CUMPLIDA", "✗ NO CUMPLIDA"), "\n")
cat("Propiedad b):", ifelse(ks_test_b$p.value > 0.05, "✓ CUMPLIDA", "✗ NO CUMPLIDA"), "\n")
cat("Propiedad c):", ifelse(ks_test_c$p.value > 0.05, "✓ CUMPLIDA", "✗ NO CUMPLIDA"), "\n")
```
