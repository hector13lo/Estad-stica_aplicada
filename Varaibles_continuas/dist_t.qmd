...
var_teorica <- dfs / (dfs - 2)

cat("Teórica: Media =", media_teorica, ", Varianza =", var_teorica, "\n\n")
...
for (n in sample_sizes) {
  ...
  cat("Media muestral:", round(media_muestral,3),
      "| Varianza muestral:", round(var_muestral,3), "\n\n")
  
  resultados[[as.character(n)]] <- muestra
}


muestra_grande <- resultados[["1000"]]
x_vals <- seq(min(muestra_grande)-1, max(muestra_grande)+1, length.out = 200)

df_hist <- data.frame(x = muestra_grande)

ggplot(df_hist, aes(x = x)) +
  geom_histogram(aes(y = ..density..), bins = 50, fill = "blue", alpha = 0.6, color = "black") +

  stat_function(fun = function(x) dt(x, df = dfs), color = "grey", size = 1.2) +

  stat_function(fun = dnorm, args = list(mean = 0, sd = 1), color = "yellow", size = 1.2, linetype = "dashed") +
  labs(title = paste("Histograma de t(", dfs, "), n = 1000", sep=""),
       x = "Valores", y = "Densidad",
       subtitle = "Rojo: t teórica | Verde: Normal estándar") +
  theme_minimal()
...
x_vals <- seq(min(T)-1, max(T)+1, length.out = 200)
dens_teorica <- dt(x_vals, df = df)

df_hist <- data.frame(T = T)
df_dens <- data.frame(x = x_vals, dens = dens_teorica)

ggplot(df_hist, aes(x = T)) +
  geom_histogram(aes(y = ..density..), bins = 40,
                 fill = "skyblue", color = "grey", alpha = 0.7) +
  geom_line(data = df_dens, aes(x = x, y = dens), color = "orange", size = 1.2) +
  labs(title = paste("Histograma de T y densidad teórica t(", df, ")", sep=""),
       x = "Valores de T", y = "Densidad",
       subtitle = "Rojo: densidad teórica t(10)") +
  theme_minimal()


cat("Media simulada de T:", round(mean(T),3), "\n")
cat("Varianza simulada de T:", round(var(T),3), "\n")
cat("Media teórica t(10): 0, Varianza teórica t(10):", df/(df-2), "\n")
...
n_muestras <- 1000
tamano_muestra <- 10
grados_libertad <- tamano_muestra - 1  # 9 grados de libertad

# Vectores para almacenar resultados
medias_muestrales <- numeric(n_muestras)
desviaciones_muestrales <- numeric(n_muestras)
estadisticos_t <- numeric(n_muestras)

# Generar 1000 muestras de tamaño 10 de N(0,1)
for (i in 1:n_muestras) {
  # Generar muestra de distribución normal estándar
  muestra <- rnorm(tamano_muestra, mean = 0, sd = 1)
  ...
  desviaciones_muestrales[i] <- desviacion_muestral
  
  # Calcular estadístico t (STS)
  # t = (media_muestral - media_poblacional) / (s/√n)
  estadisticos_t[i] <- media_muestral / (desviacion_muestral / sqrt(tamano_muestra))
}

# Crear histograma del estadístico t
hist(estadisticos_t, breaks = 30, freq = FALSE, 
     main = "Comparación: Estadístico t vs Distribución t(9)",
     xlab = "Estadístico t", ylab = "Densidad",
     col = "lightblue", border = "white",
     xlim = c(-4, 4), ylim = c(0, 0.4))

# Superponer la función de densidad teórica t(9)
x <- seq(-4, 4, length.out = 1000)
y_teorica <- dt(x, df = grados_libertad)
lines(x, y_teorica, col = "red", lwd = 2, lty = 1)

# Agregar leyenda
legend("topright", 
       legend = c("Estadístico t empírico", "Distribución t(9) teórica"),
       col = c("lightblue", "red"), 
       lty = c(1, 1), lwd = c(10, 2),
       bty = "n")

# Prueba de normalidad (opcional)
# shapiro.test(estadisticos_t)  # No debería ser normal

# Estadísticas descriptivas
cat("Estadísticas descriptivas del estadístico t:\n")
cat("Media:", mean(estadisticos_t), "\n")
cat("Desviación estándar:", sd(estadisticos_t), "\n")
cat("Mínimo:", min(estadisticos_t), "\n")
cat("Máximo:", max(estadisticos_t), "\n\n")

# Comparar con distribución teórica t(9)
cat("Comparación con distribución t(9) teórica:\n")
cat("Media teórica t(9):", 0, "\n")
cat("Desviación estándar teórica t(9):", sqrt(grados_libertad/(grados_libertad-2)), "\n")

# Prueba Kolmogorov-Smirnov para comparar distribuciones
ks_test <- ks.test(estadisticos_t, "pt", df = grados_libertad)
cat("\nPrueba de Kolmogorov-Smirnov:\n")
cat("p-value:", ks_test$p.value, "\n")
if (ks_test$p.value > 0.05) {
  cat("No hay evidencia para rechazar que siguen la misma distribución\n")
} else {
  cat("Hay evidencia de diferencia entre las distribuciones\n")
}
