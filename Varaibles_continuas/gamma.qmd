...
for (n in n_values) {
  medias_muestrales <- replicate(num_muestras, {
    muestra <- rgamma(n, shape = alpha, rate = lambda)
    mean(muestra)
  })
  
  # Calcular estadísticas de las medias muestrales
  var_medias <- var(medias_muestrales)
  var_teorica_ajustada <- varianza_teorica / n
  
  # Almacenar resultados
  resultados <- rbind(resultados, data.frame(
    n = n,
    media_medias = mean(medias_muestrales),
    varianza_medias = var_medias,
    var_teorica_ajustada = var_teorica_ajustada,
    diferencia_media = abs(mean(medias_muestrales) - media_teorica),
    diferencia_varianza = abs(var_medias - var_teorica_ajustada)
  ))
}
...
geom_histogram(aes(y = ..density..), bins = 30,
                   fill = "green", color = "orange", alpha = 0.7) +
