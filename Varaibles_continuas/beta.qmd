...
var_muestrales <- numeric(length(sample_sizes))
...
for (i in seq_along(sample_sizes)) {
  ...
  var_muestrales[i] <- var(muestra)
}

# --------------------------------------------------
# Gráfica de convergencia de la media muestral
# --------------------------------------------------
df_media <- data.frame(n = sample_sizes, media = medias_muestrales)
ggplot(df_media, aes(x = n, y = media)) +
  ...
  labs(title = "Convergencia de la media muestral hacia la media teórica",
       x = "Tamaño de muestra (log scale)",
       y = "Media muestral") +
  theme_minimal()

# --------------------------------------------------
# Gráfica de convergencia de la varianza muestral
# --------------------------------------------------
df_var <- data.frame(n = sample_sizes, var = var_muestrales)
ggplot(df_var, aes(x = n, y = var)) +
  geom_point(color = "yellow", size = 3) +
  geom_line(color = "green") +
  geom_hline(yintercept = var_teorica, color = "orange", linetype = "dashed", size = 1) +
  scale_x_continuous(trans = "log10") +
...
# --------------------------------------------------
# Histograma de la muestra más grande con PDF teórica
# --------------------------------------------------
n_max <- max(sample_sizes)
muestra_grande <- rbeta(n_max, alpha, beta)
x_vals <- seq(0, 1, length.out = 200)
dens_teorica <- dbeta(x_vals, alpha, beta)
df_hist <- data.frame(x = muestra_grande)
df_dens <- data.frame(x = x_vals, dens = dens_teorica)

ggplot(df_hist, aes(x = x)) +
  geom_histogram(aes(y = ..density..), bins = 50, fill = "blue", alpha = 0.6, color = "grey") +
  geom_line(data = df_dens, aes(x = x, y = dens), color = "black", size = 1.2) +
  labs(title = paste("Histograma de muestra n =", n_max, "con PDF teórica Beta(2,5)"),
...
cat("Media teórica:", mu, "\nVarianza teórica:", sigma2, "\n\n")

# Tamaños de muestra a evaluar
sample_sizes <- c(5, 10, 30, 50, 100)
num_muestras <- 10000  # número de réplicas para medias muestrales

plots <- list()

for (n in sample_sizes) {
  
  muestras <- matrix(rbeta(num_muestras * n, alpha, beta), nrow = num_muestras, ncol = n)
  
  # Medias muestrales
  medias <- rowMeans(muestras)
  
  # Estandarizar según TCL
  medias_std <- (medias - mu) / (sigma / sqrt(n))
  
  # Histograma con densidad normal estándar superpuesta
  df_plot <- data.frame(medias_std)
  p <- ggplot(df_plot, aes(x = medias_std)) +
    geom_histogram(aes(y = ..density..), bins = 50, fill = "skyblue", alpha = 0.6, color = "yellow") +
    stat_function(fun = dnorm, args = list(mean = 0, sd = 1), color = "grey", size = 1.2) +
    labs(title = paste("TCL: n =", n),
         x = "Medias estandarizadas",
         y = "Densidad",
         subtitle = "Rojo: densidad normal estándar") +
    theme_minimal()
