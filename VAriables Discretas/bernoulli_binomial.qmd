...
```{r}
#| label: pieza_defectuosa-bernoulli
# Piezas defectuosas
p_pieza_defectuosa <- 0.02

# Crear tabla de probabilidades
bernoulli_moneda <- data.frame(
  Resultado = c("No defectuosa (0)", "Defectuosa (1)"),
  Valor = c(0, 1),
  Probabilidad = c(1-p_pieza_defectuosa, p_pieza_defectuosa)
)
...
# Parámetros
media_pieza_defectuosa <- p_pieza_defectuosa
varianza_pieza_defectuosa <- p_pieza_defectuosa * (1 - p_pieza_defectuosa)
desv_std_pieza_defectuosa <- sqrt(varianza_pieza_defectuosa)

cat("Parámetros estadísticos:\n")
cat("Media:", media_pieza_defectuosa, "\n")
cat("Varianza:", varianza_pieza_defectuosa, "\n")
cat("Desviación estándar:", desv_std_pieza_defectuosa, "\n")
```


-   

    a)  ¿Cuál es la probabilidad de obtener una pieza no defectuosa? 98%

-   

    b)  Calcule la media y varianza de esta distribución Parámetros estadísticos: Media: 0.02  Varianza: 0.0196 Desviación estándar: 0.14  
...
#| label: binomial-monedas
# Parámetros
n_piezas_defectuosas <- 10
p_piezas_defectuosas <- 0.02

# Calcular probabilidades
k_values <- 0:n_piezas_defectuosas
prob_binomial <- dbinom(k_values, n_piezas_defectuosas, p_piezas_defectuosas)

# Crear data frame
binomial_piezas_defectuosas <- data.frame(
  Num_piezas_defectuosas = k_values,
  Probabilidad = prob_binomial
)

# Mostrar tabla
kable(binomial_piezas_defectuosas, 
      caption = paste("Distribución Binomial - 10 pruebas de piezas (n =", n_piezas_defectuosas, ", p =", p_piezas_defectuosas, ")"),
      digits = 4,
      col.names = c("Número de piezas defectuosas", "Probabilidad")) %>%
  kable_styling(bootstrap_options = "striped")

# Parámetros estadísticos
media_piezas_defectuosas <- n_piezas_defectuosas * p_piezas_defectuosas
varianza_piezas_defectuosas <- n_piezas_defectuosas * p_piezas_defectuosas * (1 - p_piezas_defectuosas)
desv_std_piezas_defectuosas <- sqrt(varianza_piezas_defectuosas)

cat("\nParámetros estadísticos:\n")
cat("Media:", media_piezas_defectuosas, "\n")
cat("Varianza:", varianza_piezas_defectuosas, "\n")
cat("Desviación estándar:", round(desv_std_piezas_defectuosas, 4), "\n")
```
...
```{r}
#| label: Elegir opción correcta-bernoulli
# Opcion correcta
p_opcion_correcta <- 0.25

# Crear tabla de probabilidades
bernoulli_opcion_correcta <- data.frame(
  Resultado = c("Incorrecta (0)", "Correcta (1)"),
  Valor = c(0, 1),
  Probabilidad = c(1-p_opcion_correcta, p_opcion_correcta)
)

kable(bernoulli_opcion_correcta, 
      caption = "Distribución Bernoulli - Opción correcta (p = 0.25)",
      align = "ccc") %>%
  kable_styling(bootstrap_options = "striped")

# Parámetros
media_opcion_correcta <- p_opcion_correcta
varianza_opcion_correcta <- p_opcion_correcta * (1 - p_opcion_correcta)
desv_std_opcion_correcta <- sqrt(varianza_opcion_correcta)

cat("Parámetros estadísticos:\n")
cat("Media:", media_opcion_correcta, "\n")
cat("Varianza:", varianza_opcion_correcta, "\n")
cat("Desviación estándar:", desv_std_opcion_correcta, "\n")
```
...
-   
    ...
    b)  ¿Cuál es la probabilidad de acertar? 25%
...
Se lanza una moneda 15 veces: - a) ¿Cuál es la probabilidad de obtener exactamente 8 caras? 19.64% - b) ¿Cuál es la probabilidad de obtener al menos 12 caras? 1.39%- c) Calcule la media y desviación estándar Media: 7.5 Varianza: 3.75 Desviación estándar: 1.9365
```{r}
#| label: binomial-monedas
# Parámetros
n_monedas <- 15
...
Un medicamento tiene efectividad del 85%. Se administra a 20 pacientes: - a) ¿Cuál es la probabilidad de que se curen exactamente 18 pacientes? 22.23% - b) ¿Cuál es la probabilidad de que se curen entre 15 y 19 pacientes? 36.61% - c) ¿Cuántos pacientes se espera que se curen? 17 pacientes
:::
```{r}
#| label: aplicacion-medicina
# Parámetros
...
prob_15_19 <- pbinom(19, n_pacientes, p_cura) - pbinom(17, n_pacientes, p_cura)
...
ggplot(medicina_data, aes(x = k, y = probabilidad)) +
  geom_col(fill = "darkblue", alpha = 0.7, width = 0.8) +
  geom_vline(xintercept = media_curas, color = "red", linetype = "dashed", size = 1) +
  geom_vline(xintercept = c(15, 19), color = "orange", linetype = "dotted", size = 1) +
  annotate("text", x = media_curas + 1.5, y = max(prob_medicina) * 0.8, 
           label = paste("E[X] =", media_curas), color = "red") +
...
:::
```{r}
# Demostración: Suma de variables binomiales independientes
set.seed(123)

# Parámetros
n <- 10      # parámetro para X ~ Bin(n, p)
m <- 15      # parámetro para Y ~ Bin(m, p)
p <- 0.3     # probabilidad de éxito común
num_sim <- 10000  # número de simulaciones

# Simular X y Y independientes
X <- rbinom(num_sim, n, p)
Y <- rbinom(num_sim, m, p)
...
# Valores teóricos para Bin(n+m, p)
media_teo <- (n + m) * p
var_teo <- (n + m) * p * (1 - p)
...
cat("X ~ Bin(", n, ",", p, ")\n")
cat("Y ~ Bin(", m, ",", p, ")\n")
cat("X + Y debería seguir Bin(", n+m, ",", p, ")\n\n")

cat("ESTADÍSTICAS DE LA SUMA X + Y:\n")
cat("Media observada:", round(media_obs, 4), "| Media teórica:", media_teo, "\n")
cat("Varianza observada:", round(var_obs, 4), "| Varianza teórica:", round(var_teo, 4), "\n\n")

# Comparar distribuciones
freq_obs <- table(Z) / num_sim
k_vals <- as.numeric(names(freq_obs))
freq_teo <- dbinom(k_vals, n + m, p)

# Crear tabla de comparación
comparacion <- data.frame(
  k = k_vals,
  Frecuencia_Observada = as.numeric(freq_obs),
  Probabilidad_Teorica = freq_teo,
  Diferencia = abs(as.numeric(freq_obs) - freq_teo)
)
...
kable(comparacion, 
      caption = "Comparación: Distribución observada vs teórica para X + Y",
      digits = 4,
      col.names = c("k", "Freq. Observada", "Prob. Teórica", "Diferencia")) %>%
  kable_styling(bootstrap_options = "striped", full_width = FALSE)

# Prueba de bondad de ajuste chi-cuadrado
frec_obs_ajust <- table(factor(Z, levels = 0:(n+m)))
chi_cuadrado <- chisq.test(frec_obs_ajust, p = dbinom(0:(n+m), n+m, p))

cat("\nPRUEBA DE BONDAD DE AJUSTE (Chi-cuadrado):\n")
cat("Estadístico chi-cuadrado:", round(chi_cuadrado$statistic, 4), "\n")
cat("Valor p:", round(chi_cuadrado$p.value, 4), "\n")

if(chi_cuadrado$p.value > 0.05) {
  cat("CONCLUSIÓN: No se rechaza H0 - Los datos son consistentes con Bin(", n+m, ",", p, ")\n")
} else {
  cat("CONCLUSIÓN: Se rechaza H0 - Los datos NO son consistentes con la distribución teórica\n")
}
...
df_plot <- data.frame(
  k = rep(0:(n+m), 2),
  Probabilidad = c(dbinom(0:(n+m), n+m, p), 
                   as.numeric(table(factor(Z, levels = 0:(n+m))))/num_sim),
  Tipo = rep(c("Teórica", "Observada"), each = n+m+1)
)

ggplot(df_plot, aes(x = k, y = Probabilidad, fill = Tipo)) +
  geom_bar(stat = "identity", position = "dodge", alpha = 0.7) +
  labs(title = paste("Distribución de X + Y vs Binomial(", n+m, ",", p, ")"),
       subtitle = "Verificación del teorema: X + Y ~ Bin(n+m, p)",
       x = "Valor de k", y = "Probabilidad") +
  theme_minimal() +
  scale_fill_manual(values = c("Observada" = "steelblue", "Teórica" = "red"))
```
